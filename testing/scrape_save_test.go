...
func TestScrapeAndSave(t *testing.T) {
	...
	err := model.ProcessNewsData(50, scrapeData.ArticleLink, scrapeData.ArticleText, scrapeData.ImageURL, scrapeData.Paragraphs)
	if err != nil {
	...
	if getErr != nil {
		t.Errorf("Failed To Get Newly created news By Article Link! Reason: %v", err)
	}
